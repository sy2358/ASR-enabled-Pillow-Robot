...
def action(sentence, confidence):
    ...
    if confidence>0.6 and re.search(r"똑바로누워", sentence):
        arduinoWrite('c')
    if confidence>0.6 and re.search(r"나졸려", sentence):
        arduinoWrite('c')
    if confidence>0.6 and re.search(r"나잘래", sentence):
        arduinoWrite('c')
    if confidence>0.6 and re.search(r"취침모드", sentence):
        arduinoWrite('c')		
        ...
        arduinoWrite('f')
    elif confidence>0.6 and re.search(r"춤춰", sentence):
        arduinoWrite('f')
...
if not args.asr:
  ...
  try:
      results = r.recognize_google_cloud(audio, language = args.lang, 
                                        ...
                                        preferred_phrases=["똑바로누워","옆으로누워","엎드려","책읽어","자면","안","돼","마사지","나졸려","나잘래","졸려","잘래","독서모드","독서","모드","춤","춤춰","취침모드","취침"], show_all=True)
      result = {}
